...
const emailVerification: IEmailVerification = {
  async sendVerificationMail(to: string): Promise<any> {
    ...
    const bodyHtml = `<div style="background-color: #1c1f33; text-align: center; padding-top: 96px; padding-bottom: 32px; padding-left: 32px; padding-right: 32px;">
      <img src="${config.baseDomain}/images/squadrad-logo-with-name.png" style="width: 192px;">
      ...
      <a
      href="${config.baseDomain}/auth/verify-email/check?token=${emailVerificationToken}"
      target="_blank"
      ...
      <div style="margin-top: 96px; font-size: 10px; color: white;">
        If the above button does not work, please follow this link to complete verification: <a href="${config.baseDomain}/auth/verify-email/check?token=${emailVerificationToken}" target="_blank" style="color: #ff3d77;">
        ${config.baseDomain}/auth/verify-email/check?token=${emailVerificationToken}
        </a>
      </div>
    </div>`;
    const bodyText = `Hi, please verify your email address and let's get you onboard. Follow this link to complete verification: ${config.baseDomain}/auth/verify-email/check?token=${emailVerificationToken}`;
